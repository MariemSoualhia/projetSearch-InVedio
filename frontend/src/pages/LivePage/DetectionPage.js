...
const DetectionPage = ({ allCameras }) => {
  ...
  const [selectedCameraList, setSelectedCameraList] = useState([{}]);
    ...
    return localStorage.getItem('selectedPort'+localStorage.getItem('selectedCamera'+selectedCamera.name).name) || null;
  });  
...
const [isTimer, setTimer] = useState("off");
  ...
  useEffect(() => {
    ...
    const storedPort = localStorage.getItem('selectedPort'+'selectedPort'+localStorage.getItem('selectedCamera'+selectedCamera.name).name);
    if (storedPort) {
  ...
  useEffect(() => {
    ...
    const storedCamera = JSON.parse(localStorage.getItem('selectedCamera'+selectedCamera.name));
    if (storedCamera) {
  ...
  const FristStream = async () => {
    ...
    function playStream(videoPlayer, hostname, port, path, configuration, reportErrorCB) {
      ...
      var wsPort = (port != undefined) ? port :  localStorage.getItem('selectedPort'+allCameras[0].name);
    ...
    function sendStream(hostname, port, path, configuration, reportErrorCB) {
      ...
      var wsProt = (l.protocol == 'https:') ? 'wss://' : 'ws://';
      var wsHost = (hostname != undefined) ? hostname : l.hostname;
      var wsPort = (port != undefined) ? port : localStorage.getItem('selectedPort' + selectedCamera.name);
      var wsPath = (path != undefined) ? path : '/ws';
      if (wsPort)
        wsPort = ':' + wsPort;
      var wsUrl = wsProt + wsHost + wsPort + wsPath;
...
//const selectedCamera = JSON.parse(localStorage.getItem('selectedCamera'+selectedCamera.name));
    localStorage.setItem('selectedPort' + selectedCamera.name, response.data.port);
    ...
    if(response){
        ...
        function sendStream(hostname, port, path, configuration, reportErrorCB) {
          ...
          var wsProt = (l.protocol == 'https:') ? 'wss://' : 'ws://';
          var wsHost = (hostname != undefined) ? hostname : l.hostname;
          var wsPort = (port != undefined) ? port : localStorage.getItem('selectedPort'+localStorage.getItem('selectedCamera'+selectedCamera.name).name);
          var wsPath = (path != undefined) ? path : '/ws';
          if (wsPort)
            wsPort = ':' + wsPort;
          var wsUrl = wsProt + wsHost + wsPort + wsPath;
...
if( response){
 ...
 const drawRectangle = (side) => {
  ...
  console.log(rectangleData)
    ...
    axios.post('http://127.0.0.1:5050/start_counting', rectangleData)
    
    .then(response => {
      console.log('Coordonnées du rectangle envoyées avec succès !');
      console.log(response.data.port)
      setSelectedPort(response.data.port)
      localStorage.setItem('selectedPort'+localStorage.getItem('selectedCamera'+selectedCamera.name).name, response.data.port)
 ...
 <video id='video-player' ref={videoRef}     className={classes.stream}
    width={960}
    height={544}
    ...
    {selectedCamera&&<div>
      <FormControl component="fieldset">
        ...
        >
          ...
          <FormControlLabel value="line" control={<Radio />} label="Trippwaire" />
        </RadioGroup>
        </FormControl>
        {drawMode ==="roi" && 
 <FormGroup>
 <FormControlLabel
   control={<Checkbox checked={isTimer === 'on'} onChange={handleTimerChange} />}
   label="Timer"
 />
</FormGroup>
        }
      ...
      <List className={classes.cameraList}>
        {cameras.map((camera, index) => (
          ...
          >
            ...
            <ListItemText primary={<Typography variant="body1">{camera.name}</Typography>} />
          </ListItem>
        ))}
      </List>
    </>
